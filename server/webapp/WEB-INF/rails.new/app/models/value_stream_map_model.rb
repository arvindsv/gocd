...
class VSMPipelineInstanceModel
  ...
  def initialize(name, label, counter, stages, pdg_path_partial, stage_detail_path_partial)
    ...
    @stages = stages.map { |stage| VSMPipelineInstanceStageModel.new(stage.getName(), stage.getState().to_s, stage.getCounter(), name, counter, stage_detail_path_partial, stage.getDuration().getTotalSeconds()) }
  end
end
...
class VSMPipelineInstanceStageModel
  ...
  def initialize(name, status, counter, pipeline_name, pipeline_counter, stage_detail_path_partial, duration)
    @name = name
    @status = status
    @locator = ""
