...
describe Admin::TasksController do
  ...
  before :each do
    ...
    @task_type = fetch_external_task.getTaskType()
    @updated_payload = {:Url => "http://foo/bar"}
    @updated_task = fetch_external_task("pipeline.name", "stage.name", "job.1", "artifact1", [ConfigurationPropertyMother.create("Url", false, "http://foo/bar")])
    @subject = @updated_task
    @new_task = fetch_external_task("pipeline.name", "stage.name", "job.1", "artifact1", [ConfigurationPropertyMother.create("Url", false, nil)])

    @create_payload= {:Url => "http://foo"}
    @created_task = fetch_external_task("pipeline.name", "stage.name", "job.1", "artifact1", [ConfigurationPropertyMother.create("Url", false, "http://foo")])
  end
  ...
  describe "actions" do
    before do
      ...
      @external_artifacts_service = stub_service(:external_artifacts_service)
    end
    ...
    describe "create" do
      it "should perform plugin validation before creating a pluggable task" do
        ...
        expect(@external_artifacts_service).to receive(:validateFetchExternalArtifactTask) do |task|
          task.getConfiguration().getProperty("property1").addError("Url", "error message")
        end
