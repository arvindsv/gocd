...
class MaterialRevisionRepresenterTest {
  ...
  void 'should serialize a material revision which uses an SCM material'() {
    ...
    def expectedJSON = [
      ...
      "modifications": [
        [
          ...
          "modified_time": getDateAsString(materialRevision.getModification(0).getModifiedTime()),
          "comment": "comment2"
        ],
        [
          "_links": [
          ...
          "modified_time": getDateAsString(materialRevision.getModification(1).getModifiedTime()),
          "comment": "comment1"
        ]
      ]
    ]
    ...
    new JsonOutputWriter(result, new TestRequestContext()).forTopLevelObject { writer ->
      MaterialRevisionRepresenter.newToJSON(writer, materialRevision)
    }
  ...
  void 'should serialize a material revision which uses a dependency material'() {
    ...
    def newJSON = result.toString()

    assertThatJson(newJSON).isEqualTo(new Gson().toJson(expectedJSON))
  }
}
