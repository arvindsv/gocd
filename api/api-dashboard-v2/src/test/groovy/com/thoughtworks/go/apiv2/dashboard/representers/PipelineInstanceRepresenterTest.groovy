...
class PipelineInstanceRepresenterTest {
  ...
  @Test
  void 'OLD JSON: renders pipeline instance for pipeline that has never been executed with hal representation'() {
  ...
  @Test
  void 'OLD JSON: renders all pipeline instance with hal representation'() {
  ...
  @Test
  void 'OLD JSON: renders all pipeline instance with build_cause'() {
    def instance = PipelineModelMother.pipeline_instance_model([name  : "p1", label: "g1", counter: 5,
                                                                stages: [[name: "cruise", counter: "10", approved_by: "Anonymous"]]])
  ...
  void 'renders pipeline instance for pipeline that has never been executed with hal representation'() {
    ...
    new JsonOutputWriter(result, new TestRequestContext()).forTopLevelObject { writer ->
      PipelineInstanceRepresenter.newToJSON(writer, pipelineInstance)
    }
  ...
  void 'renders all pipeline instance with hal representation'() {
    ...
    def result = new StringWriter()
    new JsonOutputWriter(result, new TestRequestContext()).forTopLevelObject { writer ->
      PipelineInstanceRepresenter.newToJSON(writer, instance)
    }
    ...
    def map = [
      label       : 'g1', counter: Double.valueOf("5.0"), scheduled_at: getDateAsString(instance.getScheduledDate()),
      triggered_by: 'Triggered by Anonymous',
      build_cause : [approver          : 'anonymous',
                     is_forced         : true,
                     trigger_message   : "Forced by anonymous",
  ...
  void 'renders all pipeline instance with build_cause'() {
                                ...
                                [material_type: 'Mercurial',
                                 ...
                                 modifications: [
                                   [
                                     ...
                                     email_address: 'email2',
                                     revision     : '9fdcf27f16eadc362733328dd481d8a2c29915e1',
                                     modified_time: getDateAsString(materialRevisions.first().getModifications().first().getModifiedTime()),
                                     comment      : 'comment2'
                                   ],
                                   [
                                     _links       : [
                                     ...
                                     email_address: 'email1',
                                     revision     : 'eef77acd79809fc14ed82b79a312648d4a2801c6',
                                     modified_time: getDateAsString(materialRevisions.first().getModifications().last().getModifiedTime()),
                                     comment      : 'comment1'
                                   ]]
                                ],
                                [material_type: "Pipeline",
                                 ...
                                 modifications: [
                                   [_links        : [
                                    ...
                                    revision      : 'up1/1/first/1',
                                    modified_time : getDateAsString(materialRevisions.getMaterialRevision(1).getModifications().first().getModifiedTime()),
                                    pipeline_label: 'label'
                                   ]
                                 ]]
                              ]
    ...
    assertThatJson(actualJson).isEqualTo(
      [label: 'g1', counter: Double.valueOf("5.0"), scheduled_at: getDateAsString(date), triggered_by: 'Triggered by Anonymous', build_cause: expectedBuildCause])
  }
}
