...
class StageRepresenterTest {
  ...
  @Test
  void 'OLD JSON: renders stages and previous stage with hal representation'() {
  ...
  @Test
  void 'OLD JSON: renders stages without previous stage with hal representation'() {
    def stageInstance = new StageInstanceModel('stage2', '2', StageResult.Cancelled, new StageIdentifier('pipeline-name', 23, 'stage', '2'))
  ...
  void 'renders stages and previous stage with hal representation'() {
    ...
    new JsonOutputWriter(result, new TestRequestContext()).forTopLevelObject { writer ->
      StageRepresenter.newToJSON(writer, stageInstance, "pipeline-name", "2")
    }
    ...
    def expectedJson = [
      ...
      status        : StageState.Building,
      approved_by   : 'go-user',
      scheduled_at  : getDateAsString(date),
      previous_stage: [
        _links      : [
          self: [href: 'http://test.host/go/api/stages/pipeline-name/2/stage1/1']
        ],
  ...
  void 'renders stages without previous stage with hal representation'() {
    ...
    def result = new StringWriter()
    new JsonOutputWriter(result, new TestRequestContext()).forTopLevelObject { writer ->
      StageRepresenter.newToJSON(writer, stageInstance, "pipeline-name", "23")
    }
    def json = new Gson().fromJson(result.toString(), Map.class)
